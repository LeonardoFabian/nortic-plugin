  ...
  render() {
    ...
    return (
      <PluginSidebar
        ...
        icon="menu-alt"
        title={__("Certificate Metadata", "nortic-plugin")}
      >
        <PanelBody title={__("Information", "nortic-plugin")}>
          {is_expired && (
            <Typography variant="body2" color="text.secondary">
              {__("the certificate has expired.", "nortic-plugin")}
            </Typography>
          )}
          <TextControl
          ...
          />
          <TextareaControl
            label={__("Certificate URL", "nortic-plugin")}
            value={url}
            onChange={(url) =>
              editPost({
                meta: {
        ...
        <PanelBody title={__("Embed content", "nortic-plugin")}>
          <TextareaControl
            label={__("Embed certificate", "nortic-plugin")}
            value={iframe}
            onChange={(iframe) =>
              editPost({
                meta: {
