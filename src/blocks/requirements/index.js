...
registerBlockType(block.name, {
  ...
  edit({ attributes, setAttributes, context }) {
    ...
    const updateRequirements = (newValue) => {
      setMeta({ ...meta, requirements: newValue });
    };
    ...
    if (requirements === undefined) {
      return (
        ...
        >
          ...
          <p>
            {__(
              "This post type does not have a meta requirements field.",
              "nortic-plugin"
            )}
          </p>
        </div>
      ...
      return (
        <>
          ...
          <div {...blockProps}>
            {use_default_title ? (
              <h3>{__("Requirements", "nortic-plugin")}</h3>
            ) : (
              <RichText
                tagName="h3"
            ...
            <TextareaControl
              label={__("Add requirements", "nortic-plugin")}
              value={requirements}
              onChange={updateRequirements}
              help={__(
                "Click to add requirements for your post type.",
                "nortic-plugin"
              )}
            />
          </div>
